...
// ==========================================
const CONFIG = {
  // CORS 우회를 위해 직접 URL 대신 프록시 경로('/suno-proxy')를 사용합니다.
  BASE_URL: '/suno-proxy', 
...
export const saveSunoConfig = (baseUrl: string, token: string) => {
    console.log("Config is managed in source code.");
};
...
export const generateMusic = async (
  ...
  try {
    // 프록시 URL 사용 (현재 도메인 기준 상대 경로)
    // 예: /suno-proxy/api/custom_generate -> https://suno-api.../api/custom_generate
    ...
    const response = await fetch(url, {
      ...
      headers: { 
        ...
        // 중요: Authorization 헤더 제거. 
        // 비공식 API 서버는 Authorization 헤더를 받으면 CORS preflight에서 차단하거나
        // 쿠키 인증과 충돌을 일으킬 수 있습니다. 쿠키는 body에만 담습니다.
      },
      body: JSON.stringify({
        prompt: lyrics,
        tags: styleTags,
        ...
        wait_audio: false,
        // 많은 비공식 래퍼들이 body 내 'cookie' 필드를 통해 인증을 처리합니다.
        cookie: token 
      })
    });
    ...
    if (!response.ok) {
      ...
      // 만약 401/403 에러라면 쿠키 만료일 가능성이 매우 높습니다.
      if (response.status === 401 || response.status === 403) {
          throw new Error(`인증 실패 (${response.status}): 쿠키가 만료되었을 수 있습니다. CONFIG.COOKIE를 갱신해주세요. 서버 응답: ${errorText}`);
      }
    ...
    throw error; // 데모 모드 없이 실제 에러를 던짐
  }
};
...
export const checkGenerationStatus = async (taskId: string): Promise<SunoGenerationResponse> => {
  ...
  try {
    // 프록시 URL 사용
    const url = `${baseUrl}/api/get?ids=${taskId}`;

    const response = await fetch(url, {
      method: 'GET',
      headers: { 
        'Content-Type': 'application/json',
        // GET 요청에서는 body를 보낼 수 없으므로, 필요하다면 Authorization을 써야할 수도 있으나
        // CORS 문제가 발생하면 제거해야 합니다. 많은 래퍼들은 GET 조회 시에는 인증을 느슨하게 하거나
        // 쿼리 파라미터 등을 사용합니다. 일단 헤더 없이 시도합니다.
        // 만약 조회가 안되면 이 부분에 'cookie' 헤더는 못넣으므로 서버 로그를 확인해야 합니다.
      }
    });
    ...
    // 에러 발생 시 상태를 그대로 반환하기보다 에러 상태 반환
